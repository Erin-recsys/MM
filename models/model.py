...
class Proposed_model(nn.Module):
    def __init__(self, args, graph, graph_item, device, gamma=80, ablation=False):
        ...
        S_graph_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                                         "data_exist/S_graph.bin")
        ...
        print("Successfully loaded S graph from file")
    ...
    def build_model_S(self):
        ...
        for _ in range(self.layer_num_user_game):
            layer_S = 0
            if self.edge_node_weight == True:
                layer_S = GraphConv(self.hid_dim,self.hid_dim, weight=False, bias=False, allow_zero_in_degree = True)
            else:
                layer_S = dgl.nn.HeteroGraphConv({
                    'play': GraphConv(self.hid_dim,self.hid_dim, weight=False, bias=False, allow_zero_in_degree = True),
                    'played by': GraphConv(self.hid_dim,self.hid_dim, weight=False, bias=False, allow_zero_in_degree = True)
                })
            self.layers_S.append(layer_S)
        self.layers_S.to(self.device_)
    ...
    def forward(self):
        ...
        for layer in self.layers:
            if self.edge_node_weight == True:
                ...
                h['user'] = h_user*self.args.alpha+h_user_seek
                h['game'] = h_item*self.args.alpha+h_item_seek
            else:
                h = layer(self.graph,h)
        h_sub1 = h
        h1= {'user':self.user_embedding.clone(), 'game':self.item_embedding.clone()}
        for layer_S in self.layers_S:
            if self.edge_node_weight == True:
                h_user = layer_S(self.graph_item2user_S, (h1['game'],h1['user']))
                h_item = layer_S(self.graph_user2item_S, (h1['user'],h1['game']))
                h_user_seek=layer(self.graph_item2user_seek, (h['game'],h['user']),edge_weight=self.weight_edge)
                h_item_seek=layer(self.graph_user2item_seek, (h['user'],h['game']))
                h1['user'] = h_user*self.args.alpha+h_user_seek
                h1['game'] = h_item*self.args.alpha+h_item_seek
            else:
